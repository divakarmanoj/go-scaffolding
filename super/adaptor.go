...
func RequestToSuper(request *SuperRequest) *SuperModel {
	...
	return &SuperModel{Name: imports.NullStringPtr(request.Name), Age: request.Age, Address: RequestToAddress(request.Address)}
}
...
func ModelToSuper(model *SuperModel) *SuperResponse {
	...
	return &SuperResponse{ID: model.Model.ID, CreatedAt: model.Model.CreatedAt, UpdatedAt: model.Model.UpdatedAt, Name: imports.NullStringToPtr(model.Name), Age: model.Age, Address: ModelToAddress(model.Address)}
}
